#' Constructor for either the `EnsembleData` class or the `EnsembleDataDriver` class depending on input.
#'
#' A constructor for the the `EnsembleData` class or the `EnsembleDataDriver` class depending on whether the `MMod` argument is supplied. This is used to convert input data into the required form for `fit_ensemble_model_dri`.
...
EnsembleDataDriver <- function(observations, simulators, priors, MMod) {
  ...
  if (!missing(MMod)) {
    stan_data_sims <- generate_simulator_stan_data_withdrivers(observations, simulators, MMod)
  }
  else {
    stan_data_sims <- generate_simulator_stan_data_withdrivers(observations, simulators)
  }
  stan_input <- append(stan_data_sims, priors@priors_stan_input)

  if (!missing(MMod)) {
    ensemble_data <-   new('EnsembleDataDriver', stan_input = stan_input,
...
#' @slot MMod An `list` with length equal to the number of simulators.
