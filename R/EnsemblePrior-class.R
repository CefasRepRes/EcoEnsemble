...
#' 1. A `character` specifying how the priors are encoded. Currently supported priors are `'LKJ'`, `'inv_wishart'`, `'Beta'`, or `'hierarchical'` (`'hierarchical'` is only supported for the individual short-term discrepancies).
#' 2. The prior parameters for the discrepancy variances, either a `list` of length 2 or a `numeric` of length 4. See below.
#' 3. The correlation matrix parameters, either a `list` of length 2 or a `numeric` of length 4. See below.
#'
...
#'
#'   Selecting `'lkj'`, `'inv_wishart'`, `'beta'`, or `'hierarchical'` refers to setting LKJ, inverse Wishart, Beta, or hierarchical priors respectively. The variance parameters should be passed through as the second element of the `list` and the correlation parameters should be passed through as the third element. For `'lkj'`, `'inv_wishart'`, and `'beta'` selections, variances are parameterised by inverse-gamma distributions, so the second element of the `list` should itself be a `list` of length two, where each element gives the shape and scale parameters for each species (either as a single value which is the same for each species or a `numeric` with the same length as the number of species). The correlations should be in the following form:
#'  * If `'lkj'` is selected, then the third element should be a `numeric` \eqn{\eta} giving the LKJ shape parameter, such  that the probability density is given by  (Lewandowski et. al. 2009) \deqn{f(\Sigma | \eta)\propto \mathrm{det} (\Sigma)^{\eta - 1}.} Variances are parameterised by inverse-gamma distributions.
...
#'  So the third element should be a `numeric` of length 4 giving the \eqn{\alpha, \beta, \gamma, \delta} hyperparameters. respectively. NOTE: This options is only supported for the individual short-term discrepancy terms.
#'
...
#' @slot truth_params A `list` containing a prior specification for the processes on the truth \eqn{y^{(t)}}. See details of the `EnsemblePrior()` constructor. The default value is `list(10, list(10, 1), list(10, d))` where `d` is the number of species in the ensemble.
...
create_prior_stan_input <- function(d, ind_st_params, ind_lt_params, sha_st_params, sha_lt_params, truth_params){
  ...
  #JM 06/05/2022 - This is a magic check. Need to rewrite this code because this would be really hard to debug if anything goes wrong
