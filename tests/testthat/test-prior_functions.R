...
test_that("STAN correlation priors are generated", {
  ...
  expect_equal(correlation_prior(params = lkj_param,form=0,type="TEST"),STAN_data)
  ...
  expect_equal(correlation_prior(params = inv_wish_params,form=1,type="TEST"),STAN_data)
  ...
  expect_equal(correlation_prior(params = beta_params,form=2,type="TEST"),STAN_data)
})
