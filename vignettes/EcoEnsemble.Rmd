...
* `ind_st_params` - The individual short-term discrepancies $\mathbf{z}_{k}^{(t)}$. `list("lkj", list(25, 0.25), 30)` encodes that we use $\text{Inv-Gamma}(25, 0.25)}$ priors for the variances of all species and an $LKJ(30)$ prior on the correlation matrix.
...
The choice of priors for the ensemble model can have an impact on the model outputs, we can compare the fit using different priors.
...
#Now run the ensemble with much looser priors
